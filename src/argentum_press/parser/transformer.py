...
from argentum_press.parser.ast import (
    AbilitySequenceStatement, AbilityWord, AbleExpression, AbsorbAbility, ActivationRestrictionStatement, ActivationStatement, AddManaExpression, AddRemoveExpression, AffinityAbility, AfflictAbility, AmplifyAbility, AndExpression, AndOrExpression, AnnihilatorAbility, AnyColorSpecifier, AsLongAsStatement, AsStatement, AtStatement, AttacksWithExpression, AuraSwapAbility, AwakenAbility, BandingAbility, BeExpression, BeingStatement, BestowAbility, BloodthirstAbility, BushidoAbility, BuybackAbility, CantStatement, Card, CardDrawExpression, CastExpression, CastPostfixExpression, ChampionAbility, ChangeZoneExpression, ChoiceExpression, ChooseNewTargetsForCopyExpression, ColorExpression, CompoundStatement, CompoundTerminator, ConniveExpression, ControlExpression, CopyExpression, CostIncreaseStatement, CostReductionStatement, CostSequenceExpression, CreateTokenExpression, CrewAbility, CumulativeUpkeepAbility, CyclingAbility, DamageType, DamageTypeEnum, DashAbility, DashCostExpression, DealsDamageExpression, DealsDamageVariant, DeclarationExpression, DescriptionExpression, DestroyExpression, DevourAbility, DiesExpression, DiscardExpression, DoOnlyOnceEachTurnStatement, DoStatement, DontStatement, DredgeAbility, DuringStatementLeading, EachExpression, EchoAbility, EffectExpression, EmbalmAbility, EmergeAbility, EnchantAbility, EntwineAbility, EqualToExpression, EquipAbility, EscalateAbility, EternalizeAbility, EvokeAbility, ExceptStatement, ExileExpression, Expression, ExpressionStatement, FabricateAbility, FadingAbility, FirstMainPhaseDiscardStatement, FlashbackAbility, ForStatement, ForStatementNoStatement, FortifyAbility, FrenzyAbility, GainLoseExpression, GenericDeclarationExpression, GraftAbility, HarnessExpression, HexproofAbility, HiddenAgendaAbility, IfStatement, InAdditionToTypesExpression, IndefiniteSingularExpression, ItReference, JumpStartAbility, Keyword, KeywordAbility, KeywordAbilityListStatement, KickerAbility, LandwalkAbility, LegendRuleStatement, LevelUpAbility, LookExpression, LoseLifeExpression, MadnessAbility, ManaExpression, ManaRetentionStatement, ManaSpendableStatement, MayStatement, MayhemAbility, MillExpression, MiracleAbility, ModalChoice, ModalExpression, ModularAbility, MorphAbility, Name, NameReference, NamedExpression, NinjutsuAbility, NonExpression, NumberOfExpression, NumberTypeEnum, NumberValue, OfferingAbility, OrExpression, OtherThanExpression, OutlastAbility, OverloadAbility, PTExpression, PartnerAbility, PayExpression, PayManaExpression, PlayExpression, PoisonousAbility, PreventDamageExpression, PreventDamageVariantG, ProtectionAbility, ProwlAbility, PutInZoneExpression, RampageAbility, RandomOrderPlacement, RatherStatement, RecoverAbility, RedirectAllDamageExpression, RegularAbility, ReinforceAbility, RemainsExpression, ReminderText, RenownAbility, ReplicateAbility, ReturnExpression, RevealExpression, RiotAbility, RippleAbility, SacrificeExpression, ScavengeAbility, SearchLibraryExpression, SelfReference, ShuffleLibraryExpression, SimpleKeywordAbility, SoulshiftAbility, SpliceAbility, Statement, StatementBlock, SurgeAbility, SurveilAbility, SurveilExpression, SuspectExpression, SuspendAbility, TapUntapExpression, TargetExpression, TextBox, ThereExistsStatement, TransfigureAbility, TransmuteAbility, TributeAbility, TriggerRestrictionStatement, TriggeredAbility, TypeExpression, UncastExpression, UnearthAbility, UnlessStatement, UntilStatement, UpToOneTargetCreatureExpression, ValueExpression, ValueGtEqExpression, ValueLtEqExpression, VanishingAbility, WardAbility, WebSlingingAbility, WhenStatement, WheneverStatement, WhereStatement, WhoExpression, WithExpression, Zone)
...
class CardTransformer(Transformer):
    ...
    def playexpression(self, items):
        # playerdeclref? ("play"|"plays"|"played") declarationorreference timeexpression?
        # The subject (declarationorreference) is mandatory; the optional
        # player and timing arrive as already-transformed Expressions on either
        # side. Dispatch positionally by item count.
        if len(items) == 1:
            return PlayExpression(subject=items[0])
        if len(items) == 3:
            return PlayExpression(player=items[0], subject=items[1], timing=items[2])
        # len == 2: bare "play ~ this turn" is the common shape, so treat
        # the trailing item as timing rather than as an explicit player.
        return PlayExpression(subject=items[0], timing=items[1])
    def remainsexpression(self, items):
        # declarationorreference? "remain"["s"] (modifier | locationexpression)
        # Children are already transformed; the optional subject (when present)
        # is the first item, and the trailing payload is either a modifier or a
        # locationexpression result.
        subject = None
        payload = None
        if len(items) == 2:
            subject, payload = items[0], items[1]
        elif len(items) == 1:
            payload = items[0]
        if isinstance(payload, (ChangeZoneExpression, Zone)):
            return RemainsExpression(subject=subject, location=payload)
        return RemainsExpression(subject=subject, modifier=payload)
    def manaspendablestatement(self, items):
        # "mana" "of" "any" "type" "can" "be" "spent" "to" "cast" "spells" "this" "way" — literal-only marker.
        return ManaSpendableStatement()
    def diesexpression(self, items):
        # declarationorreference? "die"["s"] timeexpression?
        subject = None
        timing = None
        for item in items:
            if isinstance(item, Expression):
                if subject is None:
                    ...
                    timing = item
        return DiesExpression(subject=subject, timing=timing)
    def discardexpression(self, items):
        player = None
        subject = None
        at_random = False
        expressions = []
        ...
        if len(expressions) >= 2:
            player = expressions[0]
            subject = expressions[1]
        elif len(expressions) == 1:
            subject = expressions[0]
        return DiscardExpression(player=player, subject=subject, at_random=at_random)
    def dontstatement(self, items):
        subject = None
        statement = None
        for item in items:
            if isinstance(item, Statement):
                ...
                subject = item
        return DontStatement(subject=subject, statement=statement)
    def whoexpression(self, items):
        # "who" statement — extract the Statement child.
        statement = None
        for item in items:
            if isinstance(item, Statement):
                statement = item
        return WhoExpression(statement=statement)
    def loselifeexpression(self, items):
        # playerdeclref? "lose"["s"] (valueexpression? "life" | "life" valueexpression)
        ...
        for item in items:
            if isinstance(item, Token):
                if str(item).lower() == "lost":
                    past_tense = True
                continue
            if isinstance(item, ValueExpression):
                amount = item
            elif isinstance(item, Expression):
                expressions.append(item)
        if expressions:
            ...
            if len(expressions) > 1:
                timing = expressions[1]
        return LoseLifeExpression(player=player, amount=amount, timing=timing, past_tense=past_tense)
    def equaltoexpression(self, items):
        # "<effect>? equal to <value>" — effect is optional, value is a ValueExpression or DeclarationExpression.
        effect = None
        value = None
        for item in items:
            if isinstance(item, EffectExpression):
                effect = item
    ...
    def ratherstatement(self, items):
        # "<preferred> rather than <alternative>" — two Statement children in order.
        statements = [item for item in items if isinstance(item, Statement)]
        preferred = statements[0] if len(statements) >= 1 else None
        alternative = statements[1] if len(statements) >= 2 else None
        return RatherStatement(preferred=preferred, alternative=alternative)
    def harnessexpression(self, items):
        target = None
        for item in items:
            if isinstance(item, Expression):
                target = item
        return HarnessExpression(target=target)
    def wherestatement(self, items):
        body = None
        definition = None
        statements = [item for item in items if isinstance(item, Statement)]
        if len(statements) >= 1:
        ...
        return WhereStatement(body=body, definition=definition)
    def unlessstatement(self, items):
        body = None
        condition = None
        statements = [item for item in items if isinstance(item, Statement)]
        if len(statements) >= 1:
        ...
        return UnlessStatement(body=body, condition=condition)
    def castpostfix(self, items):
        player = None
        for item in items:
            if isinstance(item, Expression):
                player = item
        return CastPostfixExpression(player=player)
    def costreductionstatement(self, items):
        subject = None
        amount = None
        for item in items:
            if isinstance(item, Expression):
                if subject is None:
                    ...
                    amount = item
        return CostReductionStatement(subject=subject, amount=amount)
    def manaretentionstatement(self, items):
        return ManaRetentionStatement()
    def paymanaexpression(self, items):
        player = None
        mana = None
        for item in items:
            if isinstance(item, Expression):
                if player is None:
                    ...
                    mana = item
        return PayManaExpression(player=player, mana=mana)
    def dostatement(self, items):
        subject = None
        body = None
        for item in items:
            if isinstance(item, Expression):
                ...
                body = item
        return DoStatement(subject=subject, body=body)
    def uptoonetargetcreature(self, items):
        return UpToOneTargetCreatureExpression()
    def suspectexpression(self, items):
        reference = next((i for i in items if isinstance(i, Expression)), None)
        return SuspectExpression(reference=reference)
    def ableexpression(self, items):
        from argentum_press.parser.ast.statements import Statement
        reference = next((i for i in items if isinstance(i, Expression)), None)
        condition = next((i for i in items if isinstance(i, Statement)), None)
        return AbleExpression(reference=reference, condition=condition)
    def otherthanexpression(self, items):
        reference = next((i for i in items if isinstance(i, Expression)), None)
        return OtherThanExpression(reference=reference)
    def forstatementnostatement(self, items):
        declaration = next((i for i in items if isinstance(i, Expression)), None)
        return ForStatementNoStatement(declaration=declaration)
    def firstmainphasediscardstatement(self, items):
        return FirstMainPhaseDiscardStatement()
    def duringstatementleading(self, items):
        time = next((i for i in items if isinstance(i, Expression)), None)
        body = next((i for i in items if isinstance(i, Statement)), None)
        return DuringStatementLeading(time=time, body=body)
    def kwriot(self, items):
        return RiotAbility()
    def beexpression(self, items):
        modifier = next((i for i in items if isinstance(i, Expression) and not isinstance(i, (ValueExpression, DeclarationExpression))), None)
        value = next((i for i in items if isinstance(i, ValueExpression)), None)
        agents = [i for i in items if isinstance(i, DeclarationExpression)]
        agent = agents[0] if agents else None
        time = next((i for i in items if isinstance(i, Expression) and i is not modifier and i is not value and i is not agent), None)
        return BeExpression(modifier=modifier, value=value, agent=agent, time=time)
    def cantstatement(self, items):
        subject = next((i for i in items if isinstance(i, Expression)), None)
        body = next((i for i in items if isinstance(i, Statement)), None)
        return CantStatement(subject=subject, body=body)
    def preventdamagevariantg(self, items):
        return PreventDamageVariantG()
    def legendrulestatement(self, items):
        from argentum_press.parser.ast import AttacksWithExpression, ChooseNewTargetsForCopyExpression, DoOnlyOnceEachTurnStatement, Expression, LegendRuleStatement
        target = next((i for i in items if isinstance(i, Expression)), None)
        return LegendRuleStatement(target=target)
    def choosenewtargetsforcopyexpression(self, items):
        from argentum_press.parser.ast import AttacksWithExpression, ChooseNewTargetsForCopyExpression, DoOnlyOnceEachTurnStatement, Expression
        exprs = [i for i in items if isinstance(i, Expression)]
        player = exprs[0] if len(exprs) > 0 else None
        targets = exprs[1] if len(exprs) > 1 else None
        return ChooseNewTargetsForCopyExpression(player=player, targets=targets)
    def doonlyonceeachturnstatement(self, items):
        return DoOnlyOnceEachTurnStatement()
    def attackswithexpression(self, items):
        expressions = [i for i in items if isinstance(i, Expression)]
        attacker = expressions[0] if len(expressions) > 0 else None
        companion = expressions[1] if len(expressions) > 1 else None
        return AttacksWithExpression(attacker=attacker, companion=companion)
